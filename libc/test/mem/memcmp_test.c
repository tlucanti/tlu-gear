...
UTEST(memcmp_simple)
{
	...
	ASSERT_ZERO(tlu_memcmp(s1, s2, n));
	ASSERT_NOT_ZERO(tlu_memcmp(s1, s3, n));
}
			...
			for (uint size = 0; size < MAX_SIZE; size++) {
				{
					...
					ASSERT_ZERO(tlu_memcmp(s1 + off1, s2 + off2, size));
				...
				if (size != 0) {
					...
					utest_random_strings(s1 + off1, s2 + off2, size);
					s1[off1 + diff] = '*';
					ASSERT_NOT_ZERO(tlu_memcmp(s1 + off1, s2 + off2, size));
